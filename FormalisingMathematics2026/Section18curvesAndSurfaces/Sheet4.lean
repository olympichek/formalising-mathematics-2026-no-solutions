...
-- `Real.HolderConjugate p q` means that `p,q>1` are reals and `1/p+1/q=1`
example (p q : ℝ) (hp : 1 < p) (hq : 1 < q) (hpq : 1 / p + 1 / q = 1) : p.HolderConjugate q :=
  sorry

-- it's a structure
-- We have a version of Hoelder's inequality.
example (q : ℝ≥0∞)  (hpq : p.toReal.HolderConjugate q.toReal)
    (f : lp E p) (g : lp E q) :
    ∑' i : I, ‖f i‖ * ‖g i‖ ≤ ‖f‖ * ‖g‖ :=
